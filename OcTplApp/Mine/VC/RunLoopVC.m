...
- (id)init
{
    self = [super init];
    if (self){
        _runLoopV = [[RunLoopV alloc] init]; //对MyUIView进行初始化
        _runLoopV.backgroundColor = tableVC;
        _runLoopV.delegate = self; //将SecondVC自己的实例作为委托对象
    }
    return self;
}
- (void)viewDidLoad {
    ...
    [self setBaseUI:@"Runloop测试" sideVal:@"" backIvName:@"back_icon.png" navC:whiteC midFontC:deepBlackC sideFontC:whiteC];
    [self setUI];
}

- (void)setUI{
    [self.view addSubview:_runLoopV];
    //添加约束
    [self setMas];
}
- (void) setMas{
    [_runLoopV mas_makeConstraints:^(MASConstraintMaker *make) {
...
- (void)toFreshDs {
    dispatch_after(dispatch_time(DISPATCH_TIME_NOW, (int64_t)(2.0 * NSEC_PER_SEC)), dispatch_get_main_queue(), ^{
        [_runLoopV.tableV.mj_header endRefreshing];
    });
}
