...
- (id)init
{
    self = [super init];
    if (self){
        _storageV = [[StorageV alloc] init]; //对MyUIView进行初始化
        _storageV.backgroundColor = tableVC;
        _storageV.delegate = self; //将SecondVC自己的实例作为委托对象
    }
    return self;
}
- (void)viewDidLoad {
    ...
    [self setBaseUI:@"内存管理测试" sideVal:@"" backIvName:@"back_icon.png" navC:whiteC midFontC:deepBlackC sideFontC:whiteC];
    [self setUI];
}

- (void)setUI{
    [self.view addSubview:_storageV];
    //添加约束
    [self setMas];
}
- (void) setMas{
    [_storageV mas_makeConstraints:^(MASConstraintMaker *make) {
...
- (void)toFreshDs {
    dispatch_after(dispatch_time(DISPATCH_TIME_NOW, (int64_t)(2.0 * NSEC_PER_SEC)), dispatch_get_main_queue(), ^{
        [_storageV.tableV.mj_header endRefreshing];
    });
}
