...
-(void) setBoolV:(BOOL)boolV{
    objc_setAssociatedObject(self, @"boolV", @(boolV), OBJC_ASSOCIATION_ASSIGN);
}

-(BOOL)boolV{
    return objc_getAssociatedObject(self, @"boolV");
}
...
{
    // [NSStringFromSelector(sel) isEqualToString:@"toDo:"];
...
+(BOOL)resolveClassMethod:(SEL)sel{
    if(sel == @selector(toFunc:)){  //NSSelectorFromString(@"toFunc:")
        class_addMethod(objc_getMetaClass("Person"), sel, (IMP)funcArchieve, "v@:@:@:ld");
        return YES;
    }
